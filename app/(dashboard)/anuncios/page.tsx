...
import { getAllBannersAdmin, uploadBanner, updateBanner, deleteBanner } from '@/services/bannerService';
import type { ApiAnnouncement, ApiBanner } from '@/lib/schemas';
import { Trash2, Upload, ToggleLeft, ToggleRight, Plus, Video } from 'lucide-react';
...
export default function AdminAnunciosPage() {
    ...
    const handleUploadVideo = async (e: React.FormEvent) => {
        ...
        try {
            ...
            Swal.fire('Error', err instanceof Error ? err.message : 'No se pudo subir el video', 'error');
        } finally {
    ...
    const handleToggleAnnouncement = async (ann: ApiAnnouncement) => {
        try {
            ...
            Swal.fire('Error', err instanceof Error ? err.message : 'Error al actualizar', 'error');
        }
    };
    ...
    const handleCreateAnnouncement = async (e: React.FormEvent) => {
        ...
        try {
            const ann = await createAnnouncement(newImageFile, newTitle || undefined, newDescription || undefined);
            ...
            Swal.fire('Error', err instanceof Error ? err.message : 'No se pudo crear el anuncio', 'error');
        } finally {
    ...
    return (
        <div>
            ...
            <div className="flex gap-1 mb-6 border-b border-gray-200">
                <button
                    ...
                    className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px ${
                        tab === 'videos' ? 'border-[#4166e0] text-[#4166e0]' : 'border-transparent text-gray-500 hover:text-gray-700'
                    }`}
                >
                    Videos del home
                </button>
                <button
                    ...
                    className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px ${
                        tab === 'popup' ? 'border-[#4166e0] text-[#4166e0]' : 'border-transparent text-gray-500 hover:text-gray-700'
                    }`}
                >
                    Anuncio popup
                </button>
            ...
            {tab === 'videos' && (
                <div>
                    {/* Upload form */}
                    <form onSubmit={handleUploadVideo} className="bg-white rounded-lg border border-gray-200 p-5 mb-6">
                        ...
                        <div className="flex gap-3 flex-wrap items-end">
                            ...
                            <div className="flex-1 min-w-[200px]">
                                <label className="block text-sm text-gray-600 mb-1">Archivo de video (.mp4)</label>
                                <label className="flex items-center gap-2 border border-dashed border-gray-300 rounded px-3 py-2 cursor-pointer hover:border-[#4166e0] transition-colors text-sm text-gray-500">
                                    ...
                                    <span>{videoFile ? videoFile.name : 'Seleccionar video'}</span>
                                    <input
                                        type="file"
                                        accept="video/mp4"
                                        className="hidden"
                    ...
                    {bannersLoading ? (
                        ...
                        <p className="text-gray-400 text-center py-12">No hay videos cargados.</p>
                    ) : (
                            ...
                            {banners.map((b) => (
                                <div key={b.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                                    ...
                                    <div className="p-4 flex items-center justify-between">
                                        <div>
                                            <p className="font-medium text-gray-700 text-sm">{b.title ?? 'Sin título'}</p>
                                            <p className="text-xs text-gray-400">Orden: {b.displayOrder}</p>
                                        </div>
                ...
                <div>
                    <div className="flex justify-between items-center mb-4">
                        <p className="text-sm text-gray-500">
                            Solo un anuncio puede estar activo a la vez. El activo se mostrará al abrir la página.
                        </p>
                        <button
                    ...
                    {showNewForm && (
                        <form onSubmit={handleCreateAnnouncement} className="bg-white rounded-lg border border-gray-200 p-5 mb-6">
                            <h2 className="font-semibold text-gray-700 mb-4">Nuevo anuncio popup</h2>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="col-span-2">
                                    <label className="block text-sm text-gray-600 mb-1">Imagen del anuncio *</label>
                                    <div className="flex gap-3 items-start">
                                        <label className="flex-1 flex items-center gap-2 border border-dashed border-gray-300 rounded px-3 py-2 cursor-pointer hover:border-[#4166e0] transition-colors text-sm text-gray-500">
                                            ...
                                            <span>{newImageFile ? newImageFile.name : 'Seleccionar imagen'}</span>
                                            <input
                                                type="file"
                                        ...
                                        {newImagePreview && (
                                            <img src={newImagePreview} alt="preview" className="w-20 h-20 object-contain rounded border border-gray-200 shrink-0" />
                                        )}
                                    </div>
                                </div>
                                <div className="col-span-2 flex gap-3">
                                    <button type="submit" disabled={!newImageFile || creatingAnnouncement} className="bg-[#4166e0] text-white px-4 py-2 rounded text-sm font-medium hover:bg-[#3155c9] disabled:opacity-50">
                                        {creatingAnnouncement ? 'Guardando...' : 'Crear anuncio'}
                                    </button>
                                    <button type="button" onClick={() => setShowNewForm(false)} className="border border-gray-300 px-4 py-2 rounded text-sm text-gray-600 hover:bg-gray-50">
                                        Cancelar
                                    </button>
                                </div>
                            </div>
                    ...
                    {announcementsLoading ? (
                        ...
                        <p className="text-gray-400 text-center py-12">No hay anuncios creados.</p>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {announcements.map((ann) => (
                                <div key={ann.id} className={`bg-white rounded-lg border overflow-hidden ${ann.isActive ? 'border-[#4166e0]' : 'border-gray-200'}`}>
                                    {ann.isActive && (
                                    ...
                                    <div className="p-4">
                                        <p className="font-medium text-gray-700 text-sm truncate">{ann.title ?? 'Sin título'}</p>
                                        {ann.description && <p className="text-xs text-gray-400 truncate mt-0.5">{ann.description}</p>}
                                        <div className="flex items-center justify-between mt-3">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-gray-400">{ann.isActive ? 'Activo' : 'Inactivo'}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                {/* Replace image */}
                                                {replaceTarget === ann.id ? (
                                                    <div className="flex items-center gap-1">
                                                        <label className="text-xs text-[#4166e0] cursor-pointer underline">
                                                            {replaceFile ? replaceFile.name.slice(0, 12) + '...' : 'Elegir'}
                                                            <input
                                                                type="file"
                                                                accept="image/jpeg,image/png,image/webp,image/gif"
                                                                className="hidden"
                                                        ...
                                                        <button onClick={() => { setReplaceTarget(null); setReplaceFile(null); }} className="text-xs text-gray-400 hover:text-gray-600">✕</button>
                                                    </div>
                                                ) : (
                                                    <button
                                                ...
                                                <button onClick={() => handleDeleteAnnouncement(ann)} className="text-gray-400 hover:text-red-500 transition-colors">
                                                    <Trash2 className="h-4 w-4" />
                                                </button>
                                            </div>
                                        </div>
